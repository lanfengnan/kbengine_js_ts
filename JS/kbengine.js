...
KBEngine.KBEngineApp = function (kbengineArgs) {
	...
	this.reset = function () {
		...
		KBEngine.app.clientVersion = "2.5.0";
	...
	*/
	...
	this.getServerAddr = function (ip, port) {
		var serverAddr = ""
		if (KBEngine.app.args.domain != "") {
			if (port == 443) {
				serverAddr = KBEngine.app.args.loginAddr + port
			} else {
				serverAddr = KBEngine.app.args.baseAddr + port
			}
		} else {
			serverAddr = "ws://" + KBEngine.app.args.ip + ":" + port
		}
		return serverAddr;
	}
	...
	this.login_loginapp = function (noconnect) {
		if (noconnect) {
			var serverAddr = this.getServerAddr(KBEngine.app.args.ip, KBEngine.app.args.port);
	...
	this.login_baseapp = function (noconnect) {
		if (noconnect) {
			...
			KBEngine.app.connect(serverAddr);
			//这里需要调整
			//KBEngine.app.connect(KBEngine.app.baseappIp, KBEngine.app.baseappTcpPort);	
	...
	this.Client_onCreatedProxies = function (rndUUID, eid, entityType) {
		...
		if (entity == undefined) {
			...
			if (entity.setComponents != null) {
				entity.setComponents(KBEngine.moduledefs[entity.className])
			}
		}
		else {
			var entityMessage = KBEngine.bufferedCreateEntityMessages[eid];
